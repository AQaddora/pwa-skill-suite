...
findings, then sequences the per-area skills** and refuses to declare done without runtime
evidence. Covers the full catalog via the eight area skills plus `pwa-audit` and `pwa-verify`.
...
| Catalog area | Skill |
|---|---|
| §1 iOS/WebKit (P-101..P-125), §9 forms (P-901..P-908) | **`pwa-native-feel`** |
| §2 app shell (P-201..P-210) | **`pwa-shell`** |
| §3 responsive (P-301..P-310), §6 performance (P-601..P-611) | **`pwa-responsive`** |
| §4 manifest (P-401..P-417), §10 theming (P-1001..P-1005) | **`pwa-manifest`** |
| §5 service worker (P-501..P-519), §5b version-skew (P-520..P-531), §11 build-deploy (P-1101..P-1105) | **`pwa-offline`** |
...
7. **`pwa-offline`** — the SW and version-skew work **last**, because it's the highest-risk
   area and its real test (deploy A → deploy B) needs the rest of the app stable first.

Each area skill runs its own audit-scoped subset and cites its catalog IDs, so guidance and
...
Hand off to **`pwa-verify`** for the runtime pack, including the deploy A→B harness. **No
"converted" or "done" claim without a green verification report.** §12 (the meta-failures)
is exactly the set of things a green build does **not** prove — treat each as a required gate:
